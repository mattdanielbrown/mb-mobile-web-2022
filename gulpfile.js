// generated on 2022-04-27 using generator-webapp 4.0.0-8
...
function styles () {
	return src('app/styles/*.scss', { sourcemaps : !isProd })
		...
		.pipe(sass.sync(sassOpts).on('error', sass.logError))
			// .pipe(sass.postcss([autoprefixer()]))
...
if (isDev) {
	...
	// serve = series(clean, parallel(buildStyles, scripts, modernizr, fonts), startAppServer);
	serve = series(clean, parallel(buildStyles, scripts, fonts), startAppServer);
} else if (isTest) {
